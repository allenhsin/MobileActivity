...
%% test
 ...
 uncertainty = zeros(length(trainSet),1);
...
for iteration = 1 : length(trainSet)+1
    ...
    for i = 1 : length(testSet)
        ...
        dis_t = pdist([testSet(i,1:7);S_m(1,1:end);W_m(1,1:end);R_m(1,1:end);B_m(1,1:end)]);
        [C min_dist_index] = min(dis_t(1:4));
        
         if (min_dist_index-1) == test_gt(i)
            correct = correct + 1;
         else
    ...
    for j = 1: length(trainSet)
        ...
        tmp_a = pdist([trainSet(j,1:7);S_m(1,1:end);W_m(1,1:end);R_m(1,1:end);B_m(1,1:end)]);
        r_min = Inf;
        for m = 1 : 4
            for n = (m+1): 4
                   tmp_m = abs(tmp_a(m) - tmp_a(n));
                   if tmp_m < r_min
                        r_min = tmp_m;
                   end
            end
    ...
    uncertainty;
    [dummy_uncertain min_index] = min(uncertainty);
     
    uncertainty(min_index) = 10;
    
    if train_gt(min_index) == 0
        S = [S;trainSet(min_index,1:end)];
        S_m = mean(S);
    elseif train_gt(min_index) == 1
        W = [W;trainSet(min_index,1:end)];
        W_m = mean(W);
    elseif train_gt(min_index) == 2
        R = [R;trainSet(min_index,1:end)];
        R_m = mean(R);
    else
        B = [B;trainSet(min_index,1:end)];
        B_m = mean(B);
    end
end    
figure(1)
...
tried_num = 1;
while(tried_num < length(trainSet)+1)
    %test%
    correct = 0;
    for i = 1 : length(testSet)    
        dis_t = pdist([testSet(i,1:7);S_m(1,1:end);W_m(1,1:end);R_m(1,1:end);B_m(1,1:end)]);
